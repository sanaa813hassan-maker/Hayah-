# -*- coding: utf-8 -*-
# --- 💖 برنامج إدارة أتيليه حياه (الإصدار المكتمل 37.1) 💖 ---
# --- المميزات: صور + باركود + خصومات + منع تعارض + تليجرام + مسحوبات ملاك + دعم Cloudflare ---
...
# --- START: Serverless File System Fix (Vercel/Cloudflare) ---
IS_SERVERLESS = os.environ.get('VERCEL') == '1' or os.environ.get('CF_PAGES') == '1'
print(f"Running on a serverless platform: {IS_SERVERLESS}")

project_data_folder = os.path.join(base_dir, 'data')
# Use /tmp for data if on a serverless platform, otherwise use the project's data folder
data_folder = '/tmp' if IS_SERVERLESS else project_data_folder
print(f"Project data folder: {project_data_folder}")
print(f"Data folder (using /tmp if serverless): {data_folder}")

# If on a serverless platform, copy data from the repo to the temporary /tmp directory at startup
if IS_SERVERLESS:
    if not os.path.exists(data_folder):
        os.makedirs(data_folder)
    if os.path.exists(project_data_folder):
        print("Copying initial data files to /tmp for serverless environment...")
        for filename in os.listdir(project_data_folder):
            ...
            if os.path.isfile(source_file) and not os.path.exists(target_file): # Avoid re-copying
                print(f"Copying {source_file} to {target_file}")
                shutil.copy2(source_file, target_file)

# Use /tmp for uploads if on a serverless platform
UPLOAD_FOLDER = '/tmp/dress_images' if IS_SERVERLESS else os.path.join(static_dir, 'dress_images')
print(f"Upload folder: {UPLOAD_FOLDER}")
if not os.path.exists(UPLOAD_FOLDER):
    os.makedirs(UPLOAD_FOLDER)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'webp'}
# --- END: Serverless File System Fix ---
...
def add_rental():
    ...
    if request.method == 'POST':
        ...
        new_r = {
            'rental_id': str(dt.now().timestamp()), 'client_name': request.form['client_name'], 'phone': request.form['phone'],
            'dress_name': request.form['dress_name'].strip(), 'total_price': request.form['total_price'],
            'paid_amount': request.form['paid_amount'], 'due_date': request.form['due_date'], 'employee_name': emp, 'rental_timestamp': ts,
